...
echo ansible-playbook -i empty_inventory -v --private-key cluster_vars/$CLUSTER_NAME/${CLUSTER_NAME}_key.pem manage_terraform_playbook.yml \
-e cluster_name=$CLUSTER_NAME $PLAN_ONLY $LOCAL_DEPLOY $ATTRIBUTES
/usr/bin/time -f "wall time to run playbook:%e" \
ansible-playbook -i empty_inventory -v --private-key cluster_vars/$CLUSTER_NAME/${CLUSTER_NAME}_key.pem manage_terraform_playbook.yml \
-e cluster_name=$CLUSTER_NAME $PLAN_ONLY $LOCAL_DEPLOY $ATTRIBUTES
